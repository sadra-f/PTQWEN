...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def set_req_grad_first_half(self, set_to=False):
        ...
        for i, l in enumerate(self.layers[:mid_idx]):
            if i % 2 == 1:
                for p in l.parameters():
                    p.requires_grad = set_to
                l.self_attn.check_calc_pt_bias()
                
    def set_req_grad_second_half(self, set_to=False):
        mid_idx = len(self.layers) // 2
        for i, l in enumerate(self.layers[mid_idx:]):
            if i % 2 == 1:
                for p in l.parameters():
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, pt_ids_sorted):
        ...
        self.alpha = nn.Parameter(torch.tensor(0.0, device=self.device), requires_grad=True)
        # ================================================================ #
        # Initialize weights and apply final processing
        self.post_init()
...
#            missing_count = _max_pt_in_batch - len(mean_pools[bi])
        ...
        model_logits = logits + (vocabularized_pt_head * self.alpha * F.softmax(probe_logits[:,:,[1]], dim=-1))
    ...
    def freeze_lm_head(self, do_freeze=True):
        for param in self.lm_head.parameters():
